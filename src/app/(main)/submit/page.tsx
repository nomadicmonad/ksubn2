...
import { validateSourceUrl, slugify, extractDomain } from '@/lib/utils';
...
export default function SubmitPage() {
  ...
  async function ensureEntity(entity: Entity, sb: Awaited<ReturnType<typeof createClient>>): Promise<string> {
    if (!entity.id.startsWith('wikidata:')) return entity.id;
    ...
    // Queue for AI profile generation
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const { data: profile } = await sb.from('profiles').select('submissions_today, last_reset_date').eq('id', user.id).single();
      if (profile) {
        const today = new Date().toISOString().slice(0, 10);
        const count = profile.last_reset_date === today ? profile.submissions_today : 0;
        if (count >= 10) { setError('You have reached the daily limit of 10 claims. Try again tomorrow.'); setLoading(false); return; }
      }

      const [fromId, toId] = await Promise.all([
        ensureEntity(fromEntity, sb),
        ensureEntity(toEntity, sb),
      ]);
      ...
      const { error: insertError } = await sb.from('claims').insert({
        ...
        source_domain: extractDomain(sourceUrl.trim()),
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="card p-6 space-y-6">
        ...
        <div className="space-y-3">
          ...
          <EntitySearchInput label="From entity" value={fromEntity} onChange={setFromEntity} />
          <div className="flex items-center justify-center">
            <ArrowRight size={18} style={{ color: 'var(--color-text-muted)' }} />
          </div>
          <EntitySearchInput label="To entity" value={toEntity} onChange={setToEntity} />
        </div>
